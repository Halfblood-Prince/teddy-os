...
programming and poll-based runtime status tracking.
...
- `netdiag` prints IRQ, selected NIC register state, DMA buffer addresses, and packet counters
...
6. Run `netdiag` and confirm IRQ/register state, DMA addresses, and packet counters are shown for RTL8139 when that device is selected.
7. Run `dhcp`, `dns example.com`, and `fetch https://example.com` to confirm the networking surface is reachable from inside the OS.

## Known Limitations

- there is no completed packet parsing or outbound frame transmit path yet
